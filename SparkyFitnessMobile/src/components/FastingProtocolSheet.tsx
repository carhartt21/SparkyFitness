...
function getPresetCopy(
  ...
  switch (`${id}.${field}`) {
    case '16-8.name':
      return t('fastingProtocol.presets.leangains.name', {
        defaultValue: '16:8 Leangains',
      });
    case '16-8.description':
      return t('fastingProtocol.presets.leangains.description', {
        defaultValue: 'Skip breakfast and eat during an 8-hour window.',
      });
    case '18-6.name':
      return t('fastingProtocol.presets.warrior18.name', {
        defaultValue: '18:6 Warrior',
      });
    case '18-6.description':
      return t('fastingProtocol.presets.warrior18.description', {
        defaultValue: 'More aggressive fast with a 6-hour eating window.',
      });
    case '20-4.name':
      return t('fastingProtocol.presets.warrior20.name', {
        defaultValue: '20:4 Warrior',
      });
    case '20-4.description':
      return t('fastingProtocol.presets.warrior20.description', {
        defaultValue: 'Eat one large meal or spread calories over 4 hours.',
      });
    case 'circumadian.name':
      return t('fastingProtocol.presets.circadian.name', {
        defaultValue: 'Circadian Rhythm',
      });
    case 'circumadian.description':
      return t('fastingProtocol.presets.circadian.description', {
        defaultValue: 'Fast from sunset to morning.',
      });
    case 'custom.name':
      return t('fastingProtocol.presets.custom.name', {
        defaultValue: 'Custom Fast',
      });
    case 'custom.description':
      return t('fastingProtocol.presets.custom.description', {
        defaultValue: 'Set your own fasting duration.',
      });
    default:
      return fallback;
  }
  ...
  (_props, ref) => {
    ...
    useImperativeHandle(ref, () => ({
      present: (initialPresetId) => {
        setSelectedPresetId(
          initialPresetId &&
            FASTING_PRESETS.some((p) => p.id === initialPresetId)
            ? initialPresetId
            : DEFAULT_PRESET_ID
        );
      ...
      startFast(
        ...
        },
        {
          onSuccess: () => {
            ...
            Toast.show({
              type: 'success',
              text1: t('fastingProtocol.fastStarted', {
                defaultValue: 'Fast started',
              }),
            });
          },
          onError: (error) => {
            ...
            Toast.show({
              type: 'error',
              text1: t('fastingProtocol.failedStart', {
                defaultValue: 'Failed to start fast',
              }),
      ...
      >
        ...
        <BottomSheetScrollView contentContainerClassName="bg-surface px-5 pb-safe-or-8">
          ...
          {FASTING_PRESETS.map((preset) => {
            ...
            return (
              <TouchableOpacity
                ...
                accessibilityLabel={getPresetCopy(
                  t,
                  preset.id,
                  'name',
                  preset.name
              ...
              >
                <View className="flex-row items-center justify-between">
                  <Text className="text-base font-semibold text-text-primary">
                    {getPresetCopy(t, preset.id, 'name', preset.name)}
                  </Text>
                ...
                {selected && preset.id === CUSTOM_PRESET_ID && (
                  <View className="flex-row items-center mt-3">
                    <StepperInput
                      ...
                      accessibilityLabels={{
                        decrement: t(
                          'fastingProtocol.accessibility.decreaseHours',
                          { defaultValue: 'Decrease fasting duration' }
                        ),
                        input: t(
                          'fastingProtocol.accessibility.durationHours',
                          { defaultValue: 'Fasting duration in hours' }
                        ),
                        increment: t(
                          'fastingProtocol.accessibility.increaseHours',
                          { defaultValue: 'Increase fasting duration' }
                        ),
                      }}
                    />
                    <Text className="text-text-secondary text-base ml-3">
                      {t('fastingProtocol.hourFast', {
                        defaultValue: 'hour fast',
                      })}
                    </Text>
                  </View>
                )}
          ...
          <TouchableOpacity
            ...
            accessibilityRole="button"
            accessibilityLabel={t('fastingProtocol.accessibility.startTime', {
              defaultValue: 'Start time: {{time}}',
              time: startLabel,
            })}
          >
            <Text className="text-base text-text-primary">
              {t('fastingProtocol.startTime', { defaultValue: 'Start time' })}
            </Text>
          ...
          >
            {isPending
              ? t('fastingProtocol.starting', { defaultValue: 'Starting...' })
              : t('fastingProtocol.startAction', {
                  defaultValue: 'Start Fasting',
                })}
          </Button>
        </BottomSheetScrollView>
      </BottomSheetModal>
