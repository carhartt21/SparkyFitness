...
const FertilityCard: React.FC<FertilityCardProps> = ({ date }) => {
  ...
  const effectiveOvulationDate =
    ...
    predictionData?.prediction.cycles[0]?.ovulation ||
    null;
  const effectiveFertileStart =
    fertility?.fertileWindow?.[0] ||
    predictionData?.prediction.cycles[0]?.fertileStart ||
    null;
  const effectiveFertileEnd =
    fertility?.fertileWindow?.slice(-1)[0] ||
    predictionData?.prediction.cycles[0]?.fertileEnd ||
    null;
  ...
  const isFertileToday = useMemo(() => {
    ...
    if (effectiveFertileStart && effectiveFertileEnd) {
      return (
        referenceDate >= effectiveFertileStart &&
        referenceDate <= effectiveFertileEnd
      );
    }
    return false;
  ...
  const daysUntilNextPeriod = useMemo(() => {
    if (fertility?.daysUntilNextPeriod != null) {
      return fertility.daysUntilNextPeriod >= 0
        ? fertility.daysUntilNextPeriod
        : null;
    }
    ...
    <View className="bg-surface rounded-xl p-4 shadow-sm border-0 gap-3">
      ...
      <View className="flex-row justify-between">
        <View>
          <Text className="text-text-secondary text-sm">
            {t('fertility.ovulation', { defaultValue: 'Est. ovulation' })}
          </Text>
          <Text className="text-text-primary text-base font-bold">
            {effectiveOvulationDate
              ? formatDate(effectiveOvulationDate, dateLocale)
              : '—'}
          </Text>
        </View>
        <View className="items-end">
          <Text className="text-text-secondary text-sm">
            {t('fertility.nextPeriod', { defaultValue: 'Next period in' })}
          </Text>
          <Text className="text-text-primary text-base font-bold">
            {daysUntilNextPeriod != null
              ? t('fertility.days', {
                  defaultValue: '{{count}} days',
                  count: daysUntilNextPeriod,
                })
              : '—'}
          </Text>
        </View>
      ...
      {dpo !== null && (
        <View className="rounded-xl bg-raised p-3">
          <Text className="text-text-secondary text-sm mb-0.5">
            {t('fertility.twoWeekWait', { defaultValue: 'Two-week wait' })}
          </Text>
          <Text className="text-text-primary text-sm font-semibold">
            {dpo === 0
              ? t('fertility.ovulationDay', { defaultValue: 'Ovulation day' })
              : t('fertility.daysPastOvulation', {
                  defaultValue: '{{count}} days past ovulation',
                  count: dpo,
                })}
          </Text>
          {dpo >= 1 && dpo < 14 && (
            <Text className="text-text-secondary text-sm mt-1">
              {t('fertility.testAccuracy', {
                defaultValue:
      ...
      <Text className="text-text-secondary text-sm">
        {t('fertility.disclaimer', {
          defaultValue: 'Estimates from your logged data. Not medical advice.',
        })}
      </Text>
    </View>
  );
