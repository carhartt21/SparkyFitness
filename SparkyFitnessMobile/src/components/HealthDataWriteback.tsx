  ...
  const renderMetricItem = (metric: WritebackMetric) => {
    ...
    return (
      ...
      >
        ...
        <Switch
          accessibilityLabel={t('healthSync.writeMetricLabel', {
            defaultValue: 'Write {{metric}} to {{store}}',
            metric: metricLabel,
            store: storeName,
          })}
          accessibilityHint={t('healthSync.writeMetricHint', {
            defaultValue: 'Toggles writing this metric to the health store.',
          })}
  ...
  return (
    <View className="bg-surface rounded-xl p-4 mb-4 shadow-sm">
      ...
      <BottomSheetPicker<RemoveScope>
        ...
        onSelect={(scope) =>
          scope === 'all' ? onRemoveAllData() : onRemoveDateRange()
        }
        renderTrigger={({ onPress }) => (
          <Button
            variant="ghost"
            onPress={onPress}
            className="mt-2 py-1 px-0 self-start"
          >
