...
const MedicationDetailScreen: React.FC<MedicationDetailScreenProps> = ({
  ...
  const isPrn =
    ...
    med.schedules.some((s) => s.schedule_type_id === 'prn');
  ...
  const handleRemoveDose = useCallback(
    (entry: MedicationEntry) => {
      Alert.alert(
        t('medications.detail.removeDoseTitle', {
          defaultValue: 'Remove Dose',
        }),
        ...
        [
          ...
          {
            text: t('medications.detail.removeDose', {
              defaultValue: 'Remove dose',
            }),
            style: 'destructive',
            onPress: () =>
              deleteEntryMutation.mutate(entry.id, {
                onError: (error) => {
                  ...
                  Toast.show({
                    type: 'error',
                    text1: t('medications.detail.removeFailed', {
                      defaultValue: 'Failed to remove dose',
                    }),
                  });
                },
              }),
        ...
        >
              ...
              {dueDoses.length === 0 && !isPrn && (
                <Text className="text-sm text-text-muted py-2">
                  {t('medications.detail.noDoses', {
                    defaultValue: 'No doses scheduled for this day.',
                  })}
                </Text>
              )}
              {isPrn && (
                <>
                  <DoseRow
                    ...
                    onLog={() => logPrn(med)}
                    title={t('medications.detail.asNeeded', {
                  ...
                  {todayPrnDoses.map((dose) => (
                    <View
                      key={dose.id}
                      className="flex-row items-center justify-between py-2 ml-9"
                    >
                      ...
                      <TouchableOpacity
                        ...
                        activeOpacity={0.6}
                        accessibilityRole="button"
                        accessibilityLabel={t('medications.detail.removeDose', {
                          defaultValue: 'Remove dose',
                        })}
                        className="ml-3"
                      >
          ...
          <View className="bg-surface rounded-xl p-4 mb-3 shadow-sm">
            <View className="flex-row justify-between items-center mb-2">
              ...
              <TouchableOpacity
                onPress={() =>
                  navigation.navigate('MedicationScheduleForm', {
                    medicationId,
                  })
                }
                hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
                activeOpacity={0.6}
                accessibilityRole="button"
                accessibilityLabel={t('medications.detail.addSchedule', {
                  defaultValue: 'Add schedule',
                })}
              >
                <Text className="text-sm font-semibold text-accent-primary">
                  {t('medications.detail.add', { defaultValue: 'Add' })}
                </Text>
              </TouchableOpacity>
            </View>
            {med.schedules?.map((sched, index) => {
              ...
              if (sched.with_meal)
                parts.push(mealTimingLabel(sched.with_meal, t));
              if (sched.active === false)
                parts.push(
                  t('medications.detail.inactive', { defaultValue: 'Inactive' })
                );
            ...
            {(!med.schedules || med.schedules.length === 0) && (
              <Text className="text-sm self-center text-text-muted">
                {t('medications.detail.noSchedule', {
                  defaultValue: 'No schedule. Doses are logged as needed.',
                })}
              </Text>
            )}
          </View>
          ...
          {(med.prescriber || med.pharmacy || med.rx_number || med.notes) && (
            <View className="bg-surface rounded-xl p-4 mb-3 shadow-sm">
              ...
              {med.prescriber && (
                <InfoRow
                  label={t('medications.detail.prescriber', {
              ...
              {med.pharmacy && (
                <InfoRow
                  label={t('medications.detail.pharmacy', {
              ...
              {med.rx_number && (
                <InfoRow
                  label={t('medications.detail.rxNumber', {
                    defaultValue: 'Rx number',
                  })}
                  value={med.rx_number}
                />
              )}
              {med.notes && (
                <InfoRow
                  label={t('medications.detail.notes', {
                    defaultValue: 'Notes',
                  })}
                  value={med.notes}
                />
