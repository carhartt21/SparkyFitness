...
export default function OnboardingScreen({ navigation }: Props) {
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      addLog(
        `[Onboarding] Settings fetch failed for ${url}: ${message}. Trying fallback reachability...`,
        'WARNING'
      );
      ...
      const reachable =
        err instanceof TimeoutError ? false : await checkReachability(url);
      if (reachable) {
        setAuthSettings({
          trusted_origin: null,
          email: { enabled: true },
        ...
        setError(
          t('onboarding.errors.serverUnreachable', {
            defaultValue:
          ...
          <>
            ...
            {hasOidc && hasEmail && (
              <View className="flex-row items-center mb-4">
                <View
                  className="flex-1"
                  style={{ height: 1, backgroundColor: borderSubtle }}
                />
                <Text
                  className="mx-3 text-xs text-text-muted uppercase"
                  style={{ marginHorizontal: 12 }}
                >
                  {t('auth.orSignInWith', { defaultValue: 'Or sign in with' })}
                </Text>
                <View
                  className="flex-1"
                  style={{ height: 1, backgroundColor: borderSubtle }}
            ...
            {authSettings && !hasEmail && !hasOidc && (
              <View
                className="py-6 px-4 items-center bg-raised rounded-lg border"
                style={{ borderWidth: 1, borderColor: borderSubtle }}
              >
