...
const ExerciseSearchScreen: React.FC<ExerciseSearchScreenProps> = ({
  ...
  const setOwnershipFilter = useAppPreferencesStore(
    (s) => s.setExerciseSearchOwnershipFilter
  );
  ...
  const handlePreviewExternalExercise = useCallback(
  ...
  const filteredRecentExercises = useMemo(
    () => filterByOwnership(recentExercises, ownershipFilter, profile?.id),
    [recentExercises, ownershipFilter, profile?.id]
  );
  const filteredTopExercises = useMemo(
    () => filterByOwnership(topExercises, ownershipFilter, profile?.id),
    [topExercises, ownershipFilter, profile?.id]
  );
  ...
  const sections = useMemo(() => {
    const allSections: ExerciseSection[] = [
      {
        title: t('exerciseSearch.sections.recent', { defaultValue: 'Recent' }),
        data: filteredRecentExercises,
      },
      {
        title: t('exerciseSearch.sections.popular', {
          defaultValue: 'Popular',
        }),
        data: filteredTopExercises,
      },
    ];
      ...
      >
        ...
        {searchText.length > 0 && (
          <Button
            ...
            hitSlop={8}
            accessibilityLabel={t('exerciseSearch.actions.clearSearch', {
  ...
  const renderSearchResults = () => {
    ...
    if (filteredSearchResults.length === 0) {
        ...
        return (
          <StatusView
            {...ownershipFilterEmptyState({
              ...
              emptyTitle: t('exerciseSearch.filter.emptyTitle', {
                defaultValue: 'No {{noun}} in {{filter}}',
              }),
              emptySubtitle: t('exerciseSearch.filter.emptySubtitle', {
                defaultValue: 'Change the filter to see your other {{noun}}.',
              }),
              showAllLabel: t('exerciseSearch.filter.showAll', {
      ...
      return (
        <StatusView
          title={t('exerciseSearch.states.noMatches', {
  ...
  const renderSearchTab = () => {
    ...
    if (isSuggestedError) {
      return (
        <StatusView
          icon="alert-circle"
          title={t('exerciseSearch.states.failedToLoad', {
    ...
    if (sections.length === 0) {
      ...
      return (
        <StatusView
          title={t('exerciseSearch.states.searchToStart', {
            defaultValue: 'Search for an exercise to get started',
          })}
        />
      );
    }
  ...
  const renderOnlineFooter = () => {
    if (isFetchNextPageError) {
      return (
        ...
        >
          {t('exerciseSearch.actions.loadMoreFailed', {
    ...
    if (isFetchingNextPage) {
      return (
        <View
          ...
          accessibilityLabel={t('exerciseSearch.accessibility.loadingMore', {
            defaultValue: 'Loading more exercises',
          })}
        >
          <ActivityIndicator size="small" color={accentColor} />
        </View>
  ...
  const renderOnlineSearchResults = () => {
    ...
    if (isOnlineSearchError) {
      return (
        <StatusView
          icon="alert-circle"
          title={t('exerciseSearch.states.failedToSearchProvider', {
            defaultValue: 'Failed to search {{provider}}',
  ...
  const renderOnlineTab = () => {
    if (!isConnected) {
      return (
        <StatusView
          icon="cloud-offline"
          title={t('exerciseSearch.states.connectToSearchOnline', {
            defaultValue: 'Connect to a server to search online exercises',
          })}
        />
      );
    }
    ...
    if (isProvidersError) {
      return (
        <StatusView
          icon="alert-circle"
          title={t('exerciseSearch.states.failedToLoadProviders', {
    ...
    if (providers.length === 0) {
      return (
        <StatusView
          icon="globe"
          iconColor={textMuted}
          title={t('exerciseSearch.states.noProviders', {
            defaultValue: 'No online exercise providers configured',
          })}
        />
      );
    }
      ...
      <View className="flex-1">
        ...
        {isOnlineSearchActive ? (
          ...
          <StatusView
            ...
            title={t('exerciseSearch.states.searchProvider', {
              defaultValue: 'Search {{provider}} for exercises',
              provider: selectedProviderName,
            })}
          />
        )}
