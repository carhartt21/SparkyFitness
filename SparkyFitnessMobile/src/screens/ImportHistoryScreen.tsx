...
const pausedReasonCopy = (
  ...
  switch (outcome) {
    case 'quota':
      return t('importHistory.paused.quota', {
        defaultValue:
          "{{source}}'s daily history limit was reached. Your progress has been saved. Resume tomorrow to continue where you left off.",
        source: healthSourceName,
      });
    case 'device-locked':
      return t('importHistory.paused.deviceLocked', {
        defaultValue:
          'Your device locked during the import, so health data became unreadable. Unlock your device and resume.',
      });
    case 'app-inactive':
      return t('importHistory.paused.appInactive', {
        defaultValue:
          'The app went to the background during the import. Keep it open and unlocked, then resume.',
      });
    case 'server-changed':
      return t('importHistory.paused.serverChanged', {
        defaultValue:
          'The active server changed during the import. Switch back to that server to resume, or start over to import into this one.',
      });
    case 'upload-failed':
      return t('importHistory.paused.uploadFailed', {
        defaultValue:
          'Uploading to your server failed{{details}}. Check your connection and resume to retry.',
        details: error ? ` (${error})` : '',
      });
    case 'window-failed':
      return t('importHistory.paused.windowFailed', {
        defaultValue:
          'Reading health data failed{{details}}. Resume to retry from where it stopped.',
        details: error ? ` (${error})` : '',
      });
    case 'already-running':
      return t('importHistory.paused.alreadyRunningResume', {
        defaultValue:
          'Another sync is running right now. Wait a moment for it to finish, then resume.',
      });
    default:
      return null;
  }
...
const idleNoticeCopy = (
  t: TFunction,
  outcome: BackfillOutcome | null
): string | null => {
  switch (outcome) {
    case 'no-history':
      return t('importHistory.idle.noHistory', {
        defaultValue:
          'No historical data was found in {{source}} for your enabled metrics.',
        source: healthSourceName,
      });
    case 'no-metrics':
      return t('importHistory.idle.noMetrics', {
        defaultValue:
          'No metrics are enabled. Turn on the metrics you want under Health Sync first.',
      });
    case 'no-server':
      return t('importHistory.idle.noServer', {
        defaultValue: 'No active server is configured.',
      });
    case 'server-changed':
      return t('importHistory.idle.serverChanged', {
        defaultValue:
          'The active server changed during the import, so it stopped. Start again to import into the current server.',
      });
    case 'already-running':
      return t('importHistory.idle.alreadyRunning', {
        defaultValue:
          'Another sync is running right now. Wait a moment for it to finish, then start the import.',
      });
    default:
      return null;
  }
...
const timeRemainingLabel = (
  ...
  if (minutes < 1)
    return t('importHistory.time.underMinute', {
      defaultValue: 'Under a minute',
    });
  if (minutes < 60)
    return t('importHistory.time.minutes', {
      defaultValue: '{{formattedCount}} min',
      count: Math.round(minutes),
      formattedCount: Math.round(minutes).toLocaleString(locale),
    });
  const hours = Math.floor(minutes / 60);
  const remaining = Math.round(minutes % 60);
  if (remaining > 0)
    return t('importHistory.time.hoursMinutes', {
      defaultValue: '{{formattedHours}}h {{formattedMinutes}}m',
      hours,
      minutes: remaining,
      formattedHours: hours.toLocaleString(locale),
      formattedMinutes: remaining.toLocaleString(locale),
    });
  return t('importHistory.time.hours', {
...
const InfoNote: React.FC<{ icon?: IconName; text: string }> = ({
  icon = 'info-circle',
  text,
}) => {
...
const ProgressSummary: React.FC<ProgressSummaryProps> = ({
  ...
  const percent =
    totalDays > 0
      ? Math.min(100, Math.round((importedDays / totalDays) * 100))
      : 0;
  return (
    <View>
      <View className="flex-row items-baseline gap-2 mt-2">
        ...
        <Text className="text-xl text-text-muted">
          {t('importHistory.progress.ofDays', {
      ...
      <View className="flex-row items-center justify-between mt-6">
        <Text className="text-base text-text-primary">
          {windowStart
            ? t('importHistory.progress.around', {
                defaultValue: 'Around {{date}}',
                date: monthYearLabel(windowStart, locale),
              })
            : ' '}
        </Text>
        <Text
      ...
      <SettingsRowGroup className="mt-6 mb-0">
        <SettingsRow
          title={t('importHistory.progress.recordsWritten', {
            defaultValue: 'Records written',
          })}
          rightAccessory={
        ...
        />
        <SettingsRow
          title={t('importHistory.progress.timeRemaining', {
...
const ImportHistoryScreen: React.FC<ImportHistoryScreenProps> = () => {
  ...
  const [isHealthStoreInitialized, setIsHealthStoreInitialized] =
    useState(false);
  ...
  const importStats =
    progress?.phase === 'importing'
      ? {
          ...
          windowStart:
            progress.currentWindow?.start ??
            (checkpoint ? new Date(checkpoint.cursor) : null),
          recordsUploaded: progress.recordsUploaded,
        }
      : checkpoint
        ? {
            importedDays: countLocalDays(
              new Date(checkpoint.cursor),
              new Date(checkpoint.endEdge)
            ),
            totalDays: countLocalDays(
        ...
        {status === 'idle' && (
          <View>
            <Text className="text-text-primary text-xl py-4 font-semibold">
              {t('importHistory.idle.title', {
                defaultValue: 'Import your health history',
              })}
            </Text>
            ...
            <SettingsRowGroup className="mt-4 mb-0">
              <SettingsRow
                title={t('importHistory.idle.source', {
                  defaultValue: 'Source',
                })}
                rightAccessory={
              ...
              />
              <SettingsRow
                title={t('importHistory.idle.dataTypes', {
                  defaultValue: 'Data types enabled',
                })}
                rightAccessory={
            ...
            <InfoNote
              icon="clock"
              text={t('importHistory.idle.timing', {
                defaultValue:
                  'Takes a few minutes. You can pause any time and pick up where you left off.',
              })}
            />
            ...
            {!isHealthStoreInitialized && (
              <Text className="text-icon-danger mt-3 text-center">
                {isAndroid
                  ? t('importHistory.healthConnectUnavailable', {
                      defaultValue:
                        'Health Connect is not available. Please make sure it is installed and enabled.',
                    })
                  : t('importHistory.healthKitUnavailable', {
                      defaultValue:
            ...
            {isAndroid && (
              <Text className="text-text-muted text-sm mt-4">
                {t('importHistory.androidLimit', {
                  defaultValue:
                    'Health Connect limits how much historical data apps can read each day. If that limit is reached, the import automatically resumes later where it left off.',
                })}
              </Text>
            )}
          </View>
          ...
          <View>
            ...
            {progress?.phase === 'importing' && importStats ? (
              <ProgressSummary
                ...
                timeRemaining={
                  estimatedMsRemaining != null
                    ? timeRemainingLabel(t, locale, estimatedMsRemaining)
                    : null
                }
                paused={false}
              />
              ...
              <View className="py-8 items-center">
                ...
                <Text className="text-text-primary text-base mt-4">
                  {t('importHistory.running.scanning', {
                    defaultValue: 'Scanning your history…',
                  })}
                </Text>
                <Text className="text-text-secondary text-sm mt-1">
                  {t('importHistory.running.finding', {
                    defaultValue: 'Finding your earliest recorded data',
                  })}
                </Text>
              </View>
            )}
            {isAndroid && progress?.historyAccessGranted === false && (
              <Text className="text-text-muted text-xs mt-3">
                {t('importHistory.running.limitedAccess', {
                  defaultValue:
                    'Access to all past data was not granted, so the import can only reach about 30 days back. Grant it from Health Connect settings and start over to go further.',
                })}
              </Text>
            )}
            <InfoNote
              text={t('importHistory.running.keepOpen', {
                defaultValue:
            ...
            >
              <Text className="text-text-primary text-lg font-semibold">
                {pauseRequested
                  ? t('importHistory.actions.pausing', {
                      defaultValue: 'Pausing…',
                    })
                  : t('importHistory.actions.pause', {
                      defaultValue: 'Pause Import',
                    })}
              </Text>
            </Button>
            {pauseRequested && (
              <Text className="text-text-muted text-xs mt-2 text-center">
                {t('importHistory.running.finishingPause', {
                  defaultValue: 'Finishing up, then pausing.',
                })}
              </Text>
            )}
          </View>
          ...
          <View>
            ...
            {pausedReason && (
              ...
              >
                ...
                <Text className="text-text-warning text-sm flex-1">
                  {pausedReason}
                </Text>
              </View>
            )}
            {frozenSelectionDiffers && (
              <Text
                testID="metric-selection-notice"
                className="text-text-muted text-xs mt-3"
              >
                {t('importHistory.interrupted.selectionChanged', {
                  defaultValue:
                    'Your metric selection has changed since this import started. Resume continues with the original selection and Start Over uses the current one.',
                })}
              </Text>
            )}
            <InfoNote
              text={t('importHistory.interrupted.savedDays', {
                defaultValue:
                  'Days already imported are saved. Starting over discards them and re-imports from day 1.',
              })}
            />
            <Button
              className="mt-6"
            ...
            >
              <Text className="text-accent-primary text-base font-semibold">
                {t('importHistory.actions.startOver', {
          ...
          <View>
            {importStats ? (
              <View className="flex-row items-baseline gap-2 mt-2">
                ...
                <Text className="text-xl text-text-muted">
                  {t('importHistory.done.daysImported', {
            ...
            <SettingsRowGroup className="mt-6 mb-0">
              ...
              {checkpoint?.completedAt && (
                <SettingsRow
                  title={t('importHistory.done.completed', {
            ...
            <InfoNote
              text={t('importHistory.done.nextSync', {
                defaultValue:
                  'New data will be picked up automatically by normal sync. Run another import only if you enable additional health data metrics.',
              })}
            />
            <Button
              variant="ghost"
