...
An iPhone is available later in this session, but none was connected when this note was written. The Mac has an Apple Development signing identity. A development build needs a connected, trusted device and a reachable non-production server with this branch's migrations/API before the end-to-end tests can pass. Do not point a branch build at the existing production v1.7.2 API; it lacks the new endpoints.
