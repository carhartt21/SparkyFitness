...
The first physical-device build failed with Xcode error 65: no Apple account is signed in and no development provisioning profiles exist for the app, Watch, or widget bundle IDs. The Mac does have a development signing certificate, but that alone cannot install the app. The owner must sign in to Xcode Accounts with an authorized development team; then rerun the device build and continue the tests below. No iPhone app installation or camera test has passed yet.
...
- Physical-device airplane-mode and camera tests are pending because the signed build cannot yet be installed. Simulator/unit tests do not establish device durability, camera behavior, or widget cold launch.
